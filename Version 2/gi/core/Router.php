...
class Router
{
    ...
    public static function isSelf()
    {
        ...
        if (($uri == '/' || trim(CURRENT_RUN,'/')==trim($p_uri['path'],'/')) || ltrim($uri, c('url_pathinfo_delim')) == basename(CURRENT_RUN) && self::getUriQuery() == '') {
            return true;
    ...
    {
        $delimiter = c('url_pathinfo_delim');
    ...
    public static function request($query_key,&$query_result, $query_position = 0)
    {
        switch (c('url_model')) {
            case URL_COMMON:
                return Router::getRequest($query_key,$query_result);
                break;
